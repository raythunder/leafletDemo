function PointRotation(mapcenter, mapAngle) {
  this.angle = mapAngle;
  /**
   * 原始坐标旋转到指定的角度
   * @param latlng
   */
  PointRotation.prototype.pointRotation = function(latlng) {
    return covent(latlng, this.angle);
  };
   ...
   * @param latlng
   */
  PointRotation.prototype.pointTurnBack = function(latlng) {
    return covent(latlng, -this.angle);
  };

  function covent(latlng, angle) {
    if (angle == 0) return latlng;
    var dlat = latlng.lng - mapcenter.lng;
    var dlng = latlng.lat - mapcenter.lat;
    var lng = dlat * Math.cos(angle) - dlng * Math.sin(angle) + mapcenter.lng;
    var lat = dlng * Math.cos(angle) + dlat * Math.sin(angle) + mapcenter.lat;
